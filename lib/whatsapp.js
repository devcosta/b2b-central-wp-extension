...
function loadUtils() {
  ...
  window.WWebJS.getBase64FromUrl = async (url) => {
  ...
  window.WWebJS.sendWhatsappMessage = async (receiver, text, options, sendSeen, uid) => {
    try {
      ...
      if (options.media) {
        // Media already processed/passed as object (base64)
        // No action needed, options.media is already set

        // Assign text as caption if present
        if (text) {
          options.caption = text;
        }
        // Clear text content as it is now a caption, or keep undefined if no caption
        text = undefined;

      } else if (options.mediaUrl) {
        try {
          const mediaData = await window.WWebJS.getBase64FromUrl(options.mediaUrl);
          options.media = mediaData;
          // Assign text as caption if present
          if (text) {
            options.caption = text;
          }
          ...
          // Fallback or error? For now, let's error to inform user media failed
